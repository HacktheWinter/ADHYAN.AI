...
const NotificationDropdown = () => {
    ...
    const fetchNotifications = async () => {
        try {
            ...
            const results = await Promise.allSettled([
                axios.get(`http://localhost:5000/api/calendar/student/all/${studentId}`),
                axios.get(`http://localhost:5000/api/announcement/student/${studentId}`)
            ]);
            ...
            if (calendarResult.status === 'fulfilled' && calendarResult.value.data.success) {
                const eventNotifs = generateNotifications(calendarResult.value.data.events);
            ...
            if (announcementResult.status === 'fulfilled' && (announcementResult.value.data.success || Array.isArray(announcementResult.value.data.announcements))) {
                 ...
                 const recentAnnouncements = announcementNotifs.filter(n => n.time > sevenDaysAgo);
                 allNotifications = [...allNotifications, ...recentAnnouncements];
            } else if (announcementResult.status === 'rejected') {
                 console.warn("Failed to fetch announcements:", announcementResult.reason);
            }
    ...
    const generateNotifications = (events) => {
        ...
        events.forEach(event => {
            ...
            const cleanTitle = event.title.replace(/^(Quiz|Assignment|Test|Live Class):\s*/i, '');
            ...
            if (timeSinceCreation < oneDay && timeSinceCreation > 0) {
                notifs.push({
                    id: `new-${event._id}`,
                    title: cleanTitle, 
                    message: `${event.extendedProps?.type || event.type || 'Event'} published.`,
            ...
            if (timeDiff > 0 && timeDiff <= oneDay && timeDiff > oneDay - oneHour * 2) { 
                 if (timeDiff > oneHour) {
                     notifs.push({
                        id: `day-${event._id}`,
                        title: cleanTitle,
    ...
    const handleMarkAsRead = (id) => {
        ...
        localStorage.setItem(`read_notifications_${studentId}`, JSON.stringify(updated));
    };
                            ...
                            visibleNotifications.map((notif) => {
                                ...
                                return (
                                    <div 
                                        ...
                                        className={`px-4 py-3 border-b border-gray-50 last:border-0 transition-colors flex gap-3 items-start group cursor-pointer ${isRead ? 'bg-white opacity-60 hover:opacity-100' : 'bg-purple-50/30 hover:bg-purple-50/60'}`}
                                    >
                                        <div className={`mt-1 p-1.5 rounded-full flex-shrink-0 ${
                                            notif.type === 'urgent' ? 'bg-red-50' : 
                                            notif.type === 'warning' ? 'bg-amber-50' : 'bg-emerald-50'
                                        }`}>
                                            {notif.icon}
                                        </div>
                                            <div className="flex-1 min-w-0">
                                                <div className="flex justify-between items-start mb-0.5">
                                                    <div className="flex items-center gap-1.5 min-w-0 flex-1 mr-2">
                                                        {notif.className && (
                                                            <span className="font-bold text-gray-600 text-xs truncate max-w-[80px] bg-gray-100 px-1.5 py-0.5 rounded" title={notif.className}>
                                                                {notif.className}
                                                            </span>
                                                        )}
                                                        <h4 className={`text-sm font-semibold truncate ${
                                                            notif.type === 'urgent' ? 'text-red-700' : 'text-gray-900'
                                                        } ${isRead ? 'font-normal text-gray-600' : ''}`} title={notif.title}>
                                                            {notif.title}
                                                        </h4>
                                                    </div>
                                                    {!isRead && <span className="w-2 h-2 rounded-full bg-purple-500 flex-shrink-0 mt-1.5"></span>}
                                                </div>
