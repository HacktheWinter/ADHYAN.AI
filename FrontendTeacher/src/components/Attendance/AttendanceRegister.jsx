...
const AttendanceRegister = ({ classId, students }) => {
  ...
  const loadRegisterData = useCallback(async () => {
    ...
    try {
      ...
      const sessions = Array.isArray(response.data?.attendance)
        ? response.data.attendance
        : [response.data?.attendance].filter(Boolean);
        ...
        if (monthDates.includes(sessionDate)) {
          ...
          entries.forEach((entry) => {
            const existing = attendanceMap[sessionDate].find((e) => {
              ...
              const sId2 = (
                entry.studentId?._id || entry.studentId
              )?.toString();
              return sId1 === sId2;
            });
            if (existing) {
  ...
  const handleTableScroll = () => {
    ...
    if (maxScroll > 0 && customMax > 0) {
      ...
      requestAnimationFrame(() => {
        if (syncingScrollSourceRef.current === "table")
  ...
  const handleDateScrollbarScroll = () => {
    ...
    if (wrapperMax > 0 && customMax > 0) {
      ...
      requestAnimationFrame(() => {
        if (syncingScrollSourceRef.current === "bar")
          syncingScrollSourceRef.current = null;
      });
    }
  };
  ...
  const exportToXLSX = () => {
    ...
    dates.forEach((d, i) => {
      ws[XLSXStyle.utils.encode_cell({ r: 1, c: 3 + i })] = {
        v: new Date(d + "T00:00:00").toLocaleDateString("en-US", {
          weekday: "short",
        }),
        t: "s",
        s: subHdrStyle,
    ...
    filteredData.forEach((row, i) => {
      ...
      const pct =
        row.markedDays > 0
          ? Math.round((row.totalPresent / row.markedDays) * 100)
          : 0;
      ...
      ws[XLSXStyle.utils.encode_cell({ r, c: 1 })] = {
        v: row.name,
        t: "s",
        s: nameStyle,
      };
      ws[XLSXStyle.utils.encode_cell({ r, c: 2 })] = {
        v: row.email,
        t: "s",
        s: normalStyle,
      };
      ...
      ws[XLSXStyle.utils.encode_cell({ r, c: tail - 3 })] = {
        v: row.totalPresent,
        t: "n",
        s: pctGreen,
      };
      ws[XLSXStyle.utils.encode_cell({ r, c: tail - 2 })] = {
        v: row.totalAbsent,
        t: "n",
        s: pctRed,
      };
      ws[XLSXStyle.utils.encode_cell({ r, c: tail - 1 })] = {
        v: `${pct}%`,
        t: "s",
        s: pctStyle,
      };
  ...
  const handleAddHoliday = () => {
    ...
    while (cursor <= to) {
      ...
      rangeDates.push({
        ...
        groupId,
        groupFrom: newHolidayFrom,
        groupTo: newHolidayTo || newHolidayFrom,
      });
    ...
    const updated = [...filtered, ...rangeDates].sort((a, b) =>
  ...
  useEffect(() => {
    const handler = (e) => {
      if (
        showHolidayModal &&
        holidayModalRef.current &&
        !holidayModalRef.current.contains(e.target)
      ) {
  ...
  const totalWorkingDays = dates.filter(
    (d) => !isHoliday(d).isHol && new Date(d + "T00:00:00") <= today,
  ).length;
  const avgAttendance = (() => {
    const withMarked = registerData.filter((r) => r.markedDays > 0);
    if (withMarked.length === 0) return 0;
    const sum = withMarked.reduce(
      (s, r) => s + Math.round((r.totalPresent / r.markedDays) * 100),
      0,
    );
      ...
      {!isLoading && registerData.length > 0 && (
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-4">
            <div className="text-xs text-gray-400 uppercase tracking-wide mb-1 font-medium">
              Total Students
            </div>
          ...
          <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-4">
            ...
            <div
              className={`text-2xl font-extrabold ${getPctColor(avgAttendance)}`}
            >
              {avgAttendance}%
            </div>
          </div>
          ...
          >
            <div className="text-xs text-gray-400 uppercase tracking-wide mb-1 font-medium flex items-center gap-1">
              <AlertTriangle className="w-3 h-3 text-red-400" /> Below{" "}
              {lowThreshold}%
            </div>
            <div
              className={`text-2xl font-extrabold ${lowAttendanceStudents.length > 0 ? "text-red-700" : "text-gray-800"}`}
            >
        ...
        <div className="bg-red-50 border border-red-200 rounded-xl p-4 flex flex-col sm:flex-row sm:items-center gap-3">
          ...
          <div className="flex-1">
            <p className="text-sm font-bold text-red-700">
              ...
              {lowAttendanceStudents.length > 1 ? "s" : ""} below {lowThreshold}
              % attendance
            </p>
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">
            ERP style monthly attendance view
          </p>
        </div>
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden flex flex-col">
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center py-12 text-gray-400">
            ...
            <p className="font-medium text-lg text-gray-500 mb-1">
              No attendance records found
            </p>
            <p className="text-sm text-gray-400">
              Select a different month or ensure students are enrolled.
            </p>
          </div>
        ) : viewMode === "cards" ? (
            ...
            {filteredData.length === 0 ? (
              ...
              filteredData.map((row) => {
                ...
                return (
                  <div
                    key={row.id}
                    className={`rounded-xl border-2 p-4 transition-all ${isLow ? "border-red-200 bg-red-50/30" : "border-gray-100 bg-white hover:border-purple-200"}`}
                  >
                    ...
                    <div className="flex items-center gap-2 mb-3">
                      ...
                      <div
                        className={`flex-1 rounded-lg px-3 py-2 text-center border ${getPctBg(pct)}`}
                      >
                        <div
                          className={`text-lg font-extrabold ${getPctColor(pct)}`}
                        >
                          {pct !== null ? `${pct}%` : "-"}
                        </div>
                    ...
                    <div className="grid grid-cols-7 gap-1">
                      ...
                      {["Su", "Mo", "Tu", "We", "Th", "Fr", "Sa"].map((d) => (
                        <div
                          key={d}
                          className="text-center text-[9px] font-semibold text-gray-400 uppercase pb-0.5"
                        >
                          {d}
                        </div>
                      ))}
                      ...
                      {dates.map((date) => {
                        ...
                        if (isHol) {
                          ...
                          boxClassName =
                            "bg-green-400 border border-green-500 text-green-900";
                          label = "Present";
                        } else if (status === "A") {
                          boxClassName =
                            "bg-red-400 border border-red-500 text-red-900";
                          label = "Absent";
                        }
              ...
              <div className="col-span-full mt-2 border-t border-gray-100 pt-4 space-y-3">
                ...
                <div className="flex items-center justify-between">
                  <button
                    ...
                    className="flex items-center gap-1 px-3 py-1.5 text-sm font-medium text-gray-500 hover:text-purple-700 hover:bg-purple-50 rounded-lg transition cursor-pointer border border-gray-200"
                  >
                  ...
                  <button
                    ...
                    className="flex items-center gap-1 px-3 py-1.5 text-sm font-medium text-gray-500 hover:text-purple-700 hover:bg-purple-50 rounded-lg transition cursor-pointer border border-gray-200 disabled:opacity-40 disabled:cursor-not-allowed"
                  >
                ...
                <div className="flex flex-wrap items-center justify-center gap-4 pt-3 border-t border-gray-100">
                  <div className="flex items-center gap-1.5 text-xs text-gray-500">
                    <span className="w-3 h-3 rounded bg-green-400 border border-green-500"></span>{" "}
                    Present
                  </div>
                  <div className="flex items-center gap-1.5 text-xs text-gray-500">
                    <span className="w-3 h-3 rounded bg-red-400 border border-red-500"></span>{" "}
                    Absent
                  </div>
                  <div className="flex items-center gap-1.5 text-xs text-gray-500">
                  ...
                  <div className="flex items-center gap-1.5 text-xs text-red-500">
                    <AlertTriangle className="w-3 h-3" /> Below {lowThreshold}%
                  </div>
                </div>
              </div>
            )}
          ...
          <>
              ...
              <table className="w-full text-sm text-left border-collapse border-spacing-0 whitespace-nowrap min-w-max">
                <thead className="sticky top-0 z-20 shadow-sm">
                  <tr className="bg-gray-100 text-gray-700 text-xs uppercase tracking-wider divide-x divide-gray-200 border-b-2 border-gray-300">
                    <th className="sticky left-0 bg-gray-100 z-30 py-3 px-4 font-bold w-[56px] min-w-[56px] max-w-[56px] text-center border-r-2 border-gray-300">
                      S.No
                    </th>
                    <th className="sticky left-[56px] bg-gray-100 z-30 py-3 px-6 font-bold w-[260px] min-w-[260px] max-w-[260px] border-r-2 border-gray-300">
                      Student Name
                    </th>
                    {dates.map((date) => {
                      const { isHol, name: holName } = isHoliday(date);
                      return (
                        <th
                          key={date}
                          className={`py-3 px-1.5 min-w-[44px] text-center font-bold tracking-tighter ${isHol ? "bg-orange-50 text-orange-700" : ""}`}
                          title={isHol ? holName : ""}
                        >
                          <div className="flex flex-col items-center justify-center -space-y-1">
                            <span
                              className={`text-[10px] ${isHol ? "text-orange-500" : "text-gray-400"}`}
                            >
                    ...
                    <th className="sticky right-[70px] bg-gray-100 z-30 py-3 px-2 min-w-[70px] text-center text-green-700 font-bold border-l-2 border-gray-300">
                      P
                    </th>
                    <th className="sticky right-0 bg-gray-100 z-30 py-3 px-2 min-w-[70px] text-center text-red-600 font-bold border-l border-gray-300">
                      A
                    </th>
                  </tr>
                </thead>
                  ...
                  {filteredData.length === 0 ? (
                    ...
                    filteredData.map((row, index) => {
                      ...
                      return (
                        <tr
                          key={row.id}
                          className={`hover:bg-purple-50/50 transition-colors group cursor-default ${isLow ? "bg-red-50/30" : ""}`}
                        >
                          ...
                          {dates.map((date) => {
                            ...
                            return (
                              <td
                                key={date}
                                className={`py-1 px-1 min-w-[44px] text-center border-r border-gray-100 ${isHol ? "bg-orange-50/40" : ""}`}
                              >
                                {isHol ? (
                                  <span className="inline-flex items-center justify-center w-6 h-6 text-orange-300 text-xs font-medium">
                                    –
                                  </span>
                                ) : status === "P" ? (
                                  <span className="inline-flex items-center justify-center w-6 h-6 rounded bg-green-100 text-green-700 font-bold text-xs shadow-sm border border-green-200">
                                    P
                                  </span>
                                ) : status === "A" ? (
                                  <span className="inline-flex items-center justify-center w-6 h-6 rounded bg-red-100 text-red-700 font-bold text-xs shadow-sm border border-red-200">
                                    A
                                  </span>
                                ) : (
                                  <span className="inline-flex items-center justify-center w-6 h-6 text-gray-300 text-xs font-medium">
                                    -
                                  </span>
                                )}
                              </td>
            ...
            <div className="px-5 pt-2 pb-1 border-t border-gray-100 bg-white">
              <div className="mx-0 sm:ml-[316px] sm:mr-[140px]">
                <div
                  ref={dateScrollbarRef}
                  onScroll={handleDateScrollbarScroll}
                  className="date-only-scrollbar overflow-x-auto overflow-y-hidden"
                  aria-label="Scroll date columns"
                >
                  <div
                    style={{
                      width: `${Math.max(dates.length * 44, 1)}px`,
                      height: "1px",
                    }}
                  />
                </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
          ...
          >
            <div className="flex items-center justify-between p-5 border-b border-gray-100 bg-gradient-to-r from-orange-50 to-white">
              <div>
                ...
                <p className="text-sm text-gray-500 mt-0.5">
                  Add custom holidays for this class
                </p>
              </div>
              <button
            ...
            <div className="p-5 border-b border-gray-100 space-y-3">
                ...
                <div className="flex-1">
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      if (newHolidayTo && e.target.value > newHolidayTo)
                        setNewHolidayTo("");
                    }}
              ...
              {newHolidayFrom &&
                newHolidayTo &&
                newHolidayTo >= newHolidayFrom &&
                (() => {
                  const from = new Date(newHolidayFrom + "T00:00:00");
                  const to = new Date(newHolidayTo + "T00:00:00");
                  const diff = Math.round((to - from) / 86200000) + 1;
                  return (
                    <div className="flex items-center gap-2 text-xs bg-orange-50 border border-orange-200 text-orange-700 px-3 py-2 rounded-lg font-medium">
                      ...
                      {new Date(
                        newHolidayFrom + "T00:00:00",
                      ).toLocaleDateString("en-US", {
                        month: "short",
                        day: "numeric",
                      })}
                      {diff > 1 &&
                        ` – ${new Date(newHolidayTo + "T00:00:00").toLocaleDateString("en-US", { month: "short", day: "numeric" })}`}
                      )
                    </div>
                  );
                })()}
              ...
              <button
                ...
                disabled={
                  !newHolidayFrom ||
                  (newHolidayTo && newHolidayTo < newHolidayFrom)
                }
              ...
              {groupedHolidays.length === 0 ? (
                ...
                groupedHolidays.map((g) => {
                  ...
                  return (
                    <div
                      key={g.groupId}
                      className="flex items-center justify-between px-5 py-3 hover:bg-orange-50/30 transition-colors"
                    >
            ...
            <div className="p-4 border-t border-gray-100 bg-gray-50/50">
              <p className="text-xs text-gray-400 text-center">
                Sundays are automatically marked as holidays. Custom holidays
                apply to all months.
              </p>
            </div>
          </div>
